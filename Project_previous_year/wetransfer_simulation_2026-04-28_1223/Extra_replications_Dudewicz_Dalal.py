...
# -*- coding: utf-8 -*-
"""
Experiment script to run the radiology simulation with two different
implementations of the exponential distribution.
"""
...
# Set to False to avoid excessive print statements during runs
VERBOSE = False

# Import the original Exponential_distribution function
def Exponential_distribution_original(lambdaValue):
...
# Create the modified Exponential_distribution function using j1_ant
def Exponential_distribution_modified(lambdaValue):
    j1 = np.random.uniform(0, 1)
    if (j1 == 0):
        j1 += 0.0001
    j1_ant = 1 - j1
    j2 = -math.log(j1_ant) / lambdaValue
    return j2

# Original Normal_distribution function
...
# Import the original RadiologyDept class
class RadiologyDept:
    ...
    def reset_simulation(self, K, seed=0):
        self.max_C = 20000
        self.r += 1
        # SET INPUT VALUES #
        np.random.seed(seed)  # Using the provided seed value
        ...
        self.nr_servers[0] = 3
        self.nr_servers[1] = 2 # hier aanpassen 
        self.nr_servers[2] = 4
        self.nr_servers[3] = 3 
        self.nr_servers[4] = 4  # hier aanpassen 
        ...
        for source_idx in range(self.nr_arrival_sources):
            if self.use_j1_ant:
                self.t_a[source_idx] = Exponential_distribution_modified(self.lamb[source_idx])
            else:
                self.t_a[source_idx] = Exponential_distribution_original(self.lamb[source_idx])
    ...
    def __init__(self, K, use_j1_ant=False):
        self.r = -1
        self.utilization_per_departure = []
        self.objective_function_values = []
        self.use_j1_ant = use_j1_ant
    ...
    def arrival_event(self):
        ...
        if self.use_j1_ant:
            self.t_lambda = Exponential_distribution_modified(self.lamb[self.index_arr])
        else:
            self.t_lambda = Exponential_distribution_original(self.lamb[self.index_arr])
    ...
    def departure_event(self):
        ...
        for station_idx in range(0, self.nr_stations):
            if self.n_ws[station_idx] > self.nr_servers[station_idx]:
                self.tot_n_queue_ws[self.r][station_idx] += (self.t_d[self.index_dep_station][self.index_dep_server] - self.t) * (self.n_ws[station_idx] - self.nr_servers[station_idx])
                self.tot_n_queue[self.r] += (self.t_d[self.index_dep_station][self.index_dep_server] - self.t) * (self.n_ws[station_idx] - self.nr_servers[station_idx])
        ...
        # Store objective function value if job is completed
        if curr_stat >= self.nr_workstations_job[sc_type]:
            job_id = curr_cust
            if job_id >= 1195:  # Only consider jobs after warm-up period
                obj_fn_value = self.time_system[self.r][job_id] - 10 * utilization_now
                self.objective_function_values.append(obj_fn_value)
    ...
    def output(self):
        file1 = open("Extra runs New 4_2.txt", "w")
        for i1 in range(0, self.nr_stations):
        ...
        # Return the mean objective function value
        return sum(self.objective_function_values) / len(self.objective_function_values) if self.objective_function_values else 0


def run_experiment(num_replications=20):
    """
    Run the experiment with two different exponential distribution implementations.
    
    Args:
        num_replications: Number of replications to run for each implementation
        
    Returns:
        None (writes results to a file)
    """
    # Arrays to store mean objective function values
    original_means = []
    modified_means = []
    
    # Run simulations with original exponential distribution (using j1)
    print(f"Running {num_replications} replications with original exponential distribution (using j1)...")
    for rep in range(num_replications):
        print(f"  Replication {rep+1}/{num_replications}")
        radiodept = RadiologyDept(K=1, use_j1_ant=False)
        radiodept.reset_simulation(K=1, seed=rep+1)  # Different seed for each replication
        radiodept.radiology_system()
        mean_obj = sum(radiodept.objective_function_values) / len(radiodept.objective_function_values)
        original_means.append(mean_obj)
        print(f"  Mean objective function value: {mean_obj:.6f}")
    
    # Run simulations with modified exponential distribution (using j1_ant)
    print(f"\nRunning {num_replications} replications with modified exponential distribution (using j1_ant)...")
    for rep in range(num_replications):
        print(f"  Replication {rep+1}/{num_replications}")
        radiodept = RadiologyDept(K=1, use_j1_ant=True)
        radiodept.reset_simulation(K=1, seed=rep+1)  # Same seeds as original for fair comparison
        radiodept.radiology_system()
        mean_obj = sum(radiodept.objective_function_values) / len(radiodept.objective_function_values)
        modified_means.append(mean_obj)
        print(f"  Mean objective function value: {mean_obj:.6f}")
    
    # Write results to file
    with open("simulation_results.txt", "w") as f:
        f.write("Replication\tOriginal Exponential (j1)\tModified Exponential (j1_ant)\n")
        for i in range(num_replications):
            f.write(f"{i+1}\t{original_means[i]:.6f}\t{modified_means[i]:.6f}\n")
        
        # Calculate and write overall means
        overall_original = sum(original_means) / len(original_means)
        overall_modified = sum(modified_means) / len(modified_means)
        f.write(f"\nOverall Mean\t{overall_original:.6f}\t{overall_modified:.6f}\n")
    
    print(f"\nResults saved to simulation_results.txt")


if __name__ == "__main__":
    run_experiment(20) #adjust to desired number of replications