...
The simulation operates on a weekly cyclic schedule. The department is open Monday to Saturday, with Thursday and Saturday being half-day sessions. Each day is divided into time slots of 15~minutes. Resources (nurses, doctors) are assumed to be always available.

\subsection{Overview of Research Test Design}

The study evaluates the system by varying three main factors:
\begin{enumerate}[leftmargin=2cm]
    \item \textbf{Urgent Slot Capacity:} Ranging between 10 and 17 slots. Configurations with 18 or more urgent slots were excluded from the experimental design following our preliminary analysis, which revealed that these configurations push the system beyond its capacity limits (see Section~\ref{sec:variance}).
    \item \textbf{Timing Strategy:} Three strategies for distributing urgent slots over the day/week.
    \item \textbf{Appointment Rules:} Four different single-pass appointment scheduling rules.
\end{enumerate}

Our analysis proceeds through several statistical phases: establishing steady-state behaviour (warm-up), determining batch means parameters for independent sampling, applying variance reduction (Antithetic Variables), executing a full-factorial screening design across all 96 valid configurations, and a comparative analysis to identify the best system configuration.
...
\[
    Y_k = \frac{X_k + X'_k}{2}
\]
the overall variance is substantially reduced.

Table~\ref{tab:bmm_antithetic} shows the BMM results for the same 11 representative designs after applying antithetic variates.
